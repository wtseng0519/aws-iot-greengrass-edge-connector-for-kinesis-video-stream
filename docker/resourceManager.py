...
kinesisvideo = boto3.client('kinesisvideo', region_name=REGION)
...
class resourceManager():
    ...
    def init_resources(self):
        ...
        # generate iam policy
        self.check_or_create_iam_policy()
                ...
                for key in configure:
                    ...
                    if assetConfigure.get("Type") is not None:
                        if assetConfigure.get("Type") == self.hubType:
                            ...
                            raise TypeError(
                                key + " unknown type! Please choose from EdgeConnectorForKVSHubAsset or EdgeConnectorForKVSCameraAsset")
                    else:
    ...
    def check_or_create_sitewise_asset_model(self):
        if (not self.is_sitewise_asset_model_exist(self.sitewise_asset_model_name_camera_prefix)):
            ...
            print(
                "Already have sitewise asset model for camera, skip creation and use existing model")
        ...
        if (not self.is_sitewise_asset_model_exist(self.sitewise_asset_model_name_hub_prefix)):
            ...
            print(
                "Already have sitewise asset model for hub, skip creation and use existing model")
    ...
    def is_sitewise_asset_model_exist(self, model_name):
        ...
        while True:
            if not nextTokenValue:
                ...
                response = iotsitewise.list_asset_models(
                    maxResults=50, nextToken=nextTokenValue)
            for assetModel in response.get("assetModelSummaries"):
                if model_name in assetModel.get("name"):
                    if "Hub" in model_name:
                        ...
                        self.sitewise_asset_model_id_camera = assetModel.get(
                            "id")
                    return True
    ...
    def create_sitewise_hub_asset_model(self):
        assetModelNameValue = 'EdgeConnectorForKVSHubModel-' + \
            uuid.uuid4().hex[0:4]
    ...
    def create_sitewise_camera_asset_model(self):
        assetModelNameValue = 'EdgeConnectorForKVSCameraModel-' + \
            uuid.uuid4().hex[0:4]
    ...
    def list_exists_assets(self):
        ...
        while True:
            if not nextTokenValue:
                response = iotsitewise.list_assets(
                    maxResults=100, assetModelId=self.sitewise_asset_model_id_hub)
            else:
                response = iotsitewise.list_asset_models(
                    maxResults=100, nextToken=nextTokenValue, assetModelId=self.sitewise_asset_model_id_hub)
            for hubAsset in response.get("assetSummaries"):
                self.existsHubAssets.append(hubAsset.get('name'))
                self.hubId = hubAsset.get('id')
            nextTokenValue = response.get("nextToken")
            if not nextTokenValue:
                break
        ...
        while True:
            if not nextTokenValue:
                response = iotsitewise.list_assets(
                    maxResults=100, assetModelId=self.sitewise_asset_model_id_camera)
            else:
                response = iotsitewise.list_asset_models(
                    maxResults=100, nextToken=nextTokenValue, assetModelId=self.sitewise_asset_model_id_camera)
            for cameraAsset in response.get("assetSummaries"):
                self.existsCameraAssets.append(cameraAsset.get('name'))
                self.existingCameraAssetNameAndIdMap[cameraAsset.get(
                    'name')] = cameraAsset.get('id')
            nextTokenValue = response.get("nextToken")
            if not nextTokenValue:
                break
    ...
    def check_or_create_sitewise_hub_assets(self):
        for hub in self.hubs:
            ...
            if hub.get("Name") in self.existsHubAssets:
                print("Asset" + hub.get("Name") +
                      " already exists, skip creation.")
            else:
                # create sitewise asset
                hub_asset_id = self.create_sitewise_asset(
                ...
                for key in hub:
                    if properties.get(key):
                        self.update_sitewise_property(
                            hub_asset_id, properties.get(key), hub.get(key))
                        ...
                        for child_asset_name in hub.get(key):
                            if hierarchy_id_value:
                                ...
                                while i < 5:
                                    ...
                                    if self.is_sitewise_asset_active(hub_asset_id):
                                        iotsitewise.associate_assets(
                                            ...
                                            childAssetId=self.existingCameraAssetNameAndIdMap.get(
                                                child_asset_name),
                                        )
                                        break
                                    i = i+1
                                if not self.is_sitewise_asset_active(hub_asset_id):
                                    print(
                                        "Hub asset" + hub_asset_id + "create time out, did not associate Camera assets to it")

                            else:
                                print(
                                    "Cannot find assetModelHierarchies in hub model, cannot associate camera assets to it.")
    ...
    def check_or_create_sitewise_camera_assets(self):
        for camera in self.cameras:
            if camera.get("Name") in self.existsCameraAssets:
                print("Asset" + camera.get("Name") +
                      " already exists, skip creation.")
            else:
                # create sitewise asset
                camera_asset_id = self.create_sitewise_asset(
                    camera.get("Name"), self.sitewise_asset_model_id_camera)
                # create kvs stream if not exists
                if camera.get('KinesisVideoStreamName'):
                    self.check_or_create_kinesis_video_stream(
                        camera.get('KinesisVideoStreamName'))
                else:
                    print("Cannot find KinesisVideoStreamName for " +
                          camera.get("Name") + ", will not update it")
                # create secret for rtsp url
                secret_arn = ''
                if camera.get('RTSPStream'):
                    secret_arn = self.create_secret(
                        camera.get('RTSPStream'), camera.get("Name"))
                    camera['RTSPStreamSecretARN'] = secret_arn
                else:
                    print("Cannot find RTSPStream for " +
                          camera.get("Name") + ", will not update it")
                properties = self.describe_sitewise_asset(camera_asset_id)
                for key in camera:
                    if properties.get(key):
                        self.update_sitewise_property(
                            camera_asset_id, properties.get(key), camera.get(key))

                i = 0
                while i < 5:
                    time.sleep(2)
                    if self.is_sitewise_asset_active(camera_asset_id):
                        self.turn_on_notification_on_video_upload_request_measurement(
                            camera_asset_id, properties)
                        break
                    i = i+1
                if not self.is_sitewise_asset_active(camera_asset_id):
                    print(
                        "Camera asset create time out, did not turn on VideoUploadRequest measurement notification queue")
                self.existingCameraAssetNameAndIdMap[camera.get(
                    "Name")] = camera_asset_id
    ...
    def check_or_create_kinesis_video_stream(self, streamName):
        ...
        if len(response.get('StreamInfoList')) == 0:
            ...
            print("StreamName: " + streamName +
                  " already exist, skip creation")

    # Create secret
    def create_secret(self, rtspUrl, camera_name):
        response = secretsmanager.create_secret(
            Name='CameraSecret-' + self.secret_hash(camera_name),
            Description='CameraSecret',
            SecretString='{"RTSPStreamURL": "' + rtspUrl + '"}',
        )
        return response.get('ARN')

    # Create iam policy and attach to greengrass role
    def check_or_create_iam_policy(self):
        DEFAULT_EXCHANGE_ROLE_NAME = os.environ.get('TES_ROLE_NAME')
        DEFAULT_POLICY_NAME = 'EdgeConnectorForKVSPolicy'
        EdgeConnectorForKVSPolicy = {
            "Version": "2012-10-17",
            "Statement": [
                {
                    "Action": [
        ...
        policy_arn = "arn:aws:iam::%s:policy/%s" % (
            ACCOUNT_ID, DEFAULT_POLICY_NAME)
        ...
        try:
            ...
            policy_res = iam.create_policy(
                ...
                PolicyDocument=json.dumps(EdgeConnectorForKVSPolicy)
            )
            policy_attach_res = iam.attach_role_policy(
                RoleName=DEFAULT_EXCHANGE_ROLE_NAME,
    ...
    def secret_hash(self, camera_name):
        m = hashlib.md5()
        m.update(str(camera_name).encode('utf-8'))
